<x-app-layout>
    ...
    @php
        // Ensure $page exists and is an object so template can safely use $page->...
        if (!isset($page) || is_null($page)) {
            try {
                $maxId = \Illuminate\Support\Facades\DB::table('blogs')->max('id'); // adjust table name if needed
                $nextId = is_null($maxId) ? 1 : ((int)$maxId + 1);
            } catch (\Throwable $e) {
                $nextId = 1;
            }
            $page = (object) ['id' => $nextId, 'title' => '', 'content' => '', 'blogtype_id' => '', 'slug' => '', 'meta_description' => '', 'meta_keywords' => '', 'image' => ''];
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="p-4">
                    ...
                    <input type="text" name="title" id="title" value="{{ $page->title }}" class="block mt-1 w-full" >
                ...
                <div class="p-4">
                    <x-input-label for="blogtype" :value="__('Blog Type')" />
                    <select name="blogtype" id="blogtype" class="block mt-1 w-full">
                        <option value="">{{ __('-- Select Blog Type --') }}</option>
                        @foreach(($blogtypes ?? []) as $key => $bt)
                            @php
                                // support collection of objects, associative arrays, or simple key=>label pairs
                                $value = is_object($bt) ? ($bt->id ?? $bt->value ?? $key) : (is_array($bt) ? ($bt['id'] ?? $bt['value'] ?? $key) : $key);
                                $label = is_object($bt) ? ($bt->name ?? $bt->title ?? $bt) : (is_array($bt) ? ($bt['name'] ?? $bt['title'] ?? $bt) : $bt);
                                $current = old('blogtype', $page->blogtype_id ?? $page->type ?? $page->blogtype ?? '');
                                $sel = ((string)$current === (string)$value) ? 'selected' : '';
                            @endphp
                            <option value="{{ $value }}" {{ $sel }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    <x-input-error :messages="$errors->get('blogtype')" class="mt-2" />
                </div>
                <div class="p-4">
                    <x-input-label for="slug" :value="__('Slug')" />
                    <input type="text" name="slug" id="slug" value="{{ $page->slug }}" class="block mt-1 w-full" >
                    <x-input-error :messages="$errors->get('slug')" class="mt-2" />
                </div>
                <div class="p-4">
                    <x-input-label for="meta_desc" :value="__('Meta Desc')" />
                    <input type="text" name="meta_description" id="meta_description" value="{{ $page->meta_description }}" class="block mt-1 w-full" >
                    <x-input-error :messages="$errors->get('meta_description')" class="mt-2" />
                </div>
                <div class="p-4">
                    <x-input-label for="meta_keywords" :value="__('Meta Keywords')" />
                    <input type="text" name="meta_keywords" id="meta_keywords" value="{{ $page->meta_keywords }}" class="block mt-1 w-full" >
                                ...
                                if (existingValue && existingValue.trim() !== '') {
                                    try {
                                        ...
                                        files.forEach(function(fileName) {
                                            ...
                                            dz.emit("thumbnail", mockFile, "/storage/blog/" + fileName);
                                            dz.emit("complete", mockFile);
                                            uploadedFiles.push(fileName);
                                            dz.files.push(mockFile);
                                        });

                                        // skip server fetch since we already have the image(s) from the page data
                                        return;
                                    } catch (err) {
