<x-app-layout>
    ...
    @php
        ...
        if (!isset($page) || is_null($page)) {
            try {
                $maxId = \Illuminate\Support\Facades\DB::table('services')->max('id'); // adjust table name if needed
                $nextId = is_null($maxId) ? 1 : ((int)$maxId + 1);
            } catch (\Throwable $e) {
                $nextId = 1;
            }
            $page = (object) ['id' => $nextId, 'title' => '', 'content' => '',  'slug' => '', 'meta_description' => '', 'meta_keywords' => '', 'image' => ''];
                        ...
                        Dropzone.options.imageUpload = {
                            ...
                            init: function() {
                                ...
                                this.on("removedfile", function(file) {
                                    ...
                                    $.ajax({
                                        type: 'delete',
                                        url: '{{ route('dropzone.deleteService') }}',
                                        data: {
                                            _token: '{{ csrf_token() }}',
                                            file_name: fileName,
                                            id_album: id_album
                                ...
                                if (existingValue && existingValue.trim() !== '') {
                                    try {
                                        ...
                                        files.forEach(function(fileName) {
                                            ...
                                            dz.emit("thumbnail", mockFile, "/storage/service/" + fileName);
